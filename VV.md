...
這樣的模式，修改程式需要在卡片上寫指令，一個功能就會產生許多張卡片，然後利用人工將這些卡片送進讀卡機再轉進記憶體，電腦才能執行程式。而隨著電腦速度的加快，電腦時常需要暫停，來等待人工裝卸讀卡機或者是等待列印，在等待時又不能執行其他工作，造成資源的浪費。
...
記憶單元還包含「主記憶體（Main Memory）」與「輔助記憶體」，詳細分類如圖3-13。
...
當網路中的設備要互相通訊時，必須藉由傳輸媒介才能達成資料傳輸的目的。傳輸的媒介可以從有線與無線兩大分類談起。

##### 有線傳輸媒介

1. 雙絞線
   雙絞線的英文名字叫Twist-Pair，是由兩條相互絕緣的導線按照一定的規格互相纏繞，常用於區域網路的布線，如圖3-31。

  ![](.gitbook/assets/3-31.jpg)
  
2. 同軸電纜
   同軸電纜(Coaxial cable)是一種訊號傳輸用之金屬導線，外圍以絕緣體和保護用的外皮加以包裹，如圖3-32。比雙絞線更能抵禦電磁波干擾，傳輸距離比雙絞線更長。

 ![](.gitbook/assets/3-32.jpg)

3. 光纖
   光纖（Optical fiber），是一種由玻璃或塑料製成的纖維，如圖3-33。光纖的優點是體積小、質量輕，不受電磁干擾、傳輸距離長。缺點則是成本較高，施工較為不易。
...
1. 紅外線
   紅外線（Infrared，Radiation簡稱IR）使用紅外線光束來傳輸資料，優點是成本低，缺點是傳輸距離短，且容易受到光源干擾或者障礙物阻隔。常用於無線鍵盤、無線滑鼠和點腦之間的傳輸。
2. 無線電波
   無線電波(Radio Wave)是一種電磁波，波長比紅外線更長只是保密性較差。

#### 網路設備
有了網路傳輸媒介之後，網路還需要透過各種網路設備才能相互合作完成資料的傳輸工作。目前常見的網路設備有網路卡、集線器、交換器與路由器。

##### 網路卡
網路介面控制器（network interface controller，NIC）是電腦與網路之間溝通的橋樑，如圖3-34。每一張網路卡都有一個獨一無二的媒體存取控制(Media Access Control, MAC)位址，也稱為實體位址(Phusical Address)，可以用來辨識不同的網路卡。MAC位址是由六個位元組所組成，通常會以16進位數字加以表示，如圖3-35。

![](.gitbook/assets/3-34.jpg)
![](.gitbook/assets/3-35.jpg)
##### 集線器
集線器(hub)是指將多條乙太網雙絞線或光纖集合連接在同一段物理介質下的裝置，如圖3-36。集線器下所有的電腦慧共享頻寬，因此在區域網路中，當透過集線器互相連結的電腦數量越來越多時，就逐漸會影響該電腦的效能。
![](.gitbook/assets/3-36.jpg)
##### 交換器
交換器(switch)再功能上類似集線器，但交換器會依據資料中的實體位址，而決定將資料傳送給哪一個練接埠所連接的設備，而不是像集線器把資料傳送給所有的連接設備，使用交換器可以避免不少資料的傳送，讓整體網路效能較好。
![](.gitbook/assets/3-37.jpg)
##### 路由器
路由器（Router）除了可以連接不同類型的網路區段外，它還可以根據自己路由表（Routing Table）內的資訊決定封包傳送路徑，是一種使用相當普遍的網際連接裝置。
